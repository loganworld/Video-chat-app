...
import 'package:OCWA/pages/payment/payment_completed.dart';
...
class PaymentConfirmScreen extends StatefulWidget {
  ...
  PaymentConfirmScreen({Key key, @required this.selectedContact})
      : super(key: key);
...
class _TransferConfirmItemState extends State<PaymentConfirmScreen> {
      ...
      if (personalCode != null && personalCode != "") {
        ...
        if (result2.status == 'success') {
          if (result2.data == "1") {
            await pr.show();
            pr.update(message: 'ກຳລັງດຳເນີນ...');
            final result = await NetworkUtil.post('/payment', data);
            await pr.hide();
          ...
          actions: <Widget>[
            FlatButton(
              ...
              onPressed: () {
                if (pinCode.isEmpty) {
                  Alert.info(
                      context, 'ແຈ້ງເຕືອນ', 'ກະລຸນາປ້ອນລະຫັດຢືນຢັນ', 'OK');
                  return;
                } else {
                  Navigator.of(context).pop(pinCode);
                }
